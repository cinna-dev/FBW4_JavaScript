...
router.post('/new-review', (req, res) => {
  ...
  newReview
    .save()
    .then((response) => {
      //This ".then()" will only run if the save action was successful
    ...
    .catch((error) => {
      //This will catch any error at the moment of saving the document.
      console.log(error);
      res.send({
        msg: error,
      });
...
router.put('/update-review', (req, res) => {
  ...
  MovieReviews.findByIdAndUpdate(id, newData)
    .then((response) => {
      // It is important to note that the response obtained is the old document previous to the update.
      console.log(response);
      res.send({
...
// //http://google.com/?search="javascript"&lan="english" //after (?) comes query variables

router.get('/get-reviews-movie', async (req, res) => {
  let movieID = req.query.movieid;

  let reviews = await MovieReviews.find({ movieID: movieID });
