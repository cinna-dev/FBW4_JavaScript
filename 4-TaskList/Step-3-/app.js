...
function loadEventListeners(){
    //DOM Load Event
    document.addEventListener('DOMContentLoaded',getTasks) // triggerd when Site gets reloaded
    //Add Task Event
...
function clearTasks(e){
    ...
    let deletedTasks; 
    if(localStorage.getItem('deletedTasks') == null){
        deletedTasks = JSON.parse(localStorage.getItem('tasks'));
        localStorage.setItem('deletedTasks',deletedTasks)
        
    }else{
        deletedTasks = JSON.parse(localStorage.getItem('tasks'));
        const temp = JSON.parse(localStorage.getItem('deletedTasks'))
...
////////////////////////////////////////////////
    ...
    /* const li = document.createElement('li');
    li.className = 'collection-item';
    li.appendChild(document.createTextNode(tasks[tasks.length-1]));
    const a = document.createElement('a');
    a.className = 'delete-item secondary-content';
    a.innerHTML = '<i class="fa fa-remove"></i>';
    li.appendChild(a);
    li.index = taskList.children.length
    taskList.appendChild(li); */
    removeTaskFromLocalStorage(tasks[tasks.length-1])
    const temp = document.createElement('li')
    temp.textContent = tasks[tasks.length-1]
    console.log(temp)
    storeTaskInLocalStorage(temp)
    taskInput.value = tasks[tasks.length-1];
    addTask()
    //})
